...
export const StartShowPage = () => {
  ...
  return (
    <div className="pt-10 flex flex-col items-center">
      <h1 className="text-xl font-medium text-black mb-5"> {/* Заголовок */}
        Start your showing now
      </h1>

      {data?.message && <p className="pb-3 text-red-900">{data.message}</p>} {/* Сообщение об ошибке валидации name */}

      {navigation.state === "submitting" && <Loader />} {/* Состояние сабмита, показывается лоадер */}

      <Form method="post" className="flex flex-col items-center"> {/* Компонент Form позволяет перехватить нам запрос post с помощью роутера и совершить action. Form - обёртка над for HTML-элементом, которая позволяет выполнять мутации данных, используя HTML и HTTP семантику */}
